...
horizon       = 120; %number of days horizon 120
...
%% Load data
data_table = readtable('SRISK_Data.csv');
data_table(:,1)=[];
data_table.ticker = datestr(data_table.ticker);
data_table(940:end,:)=[];
dates = datenum(data_table.ticker);


data = data_table{:,2};
for i = 3 : size(data_table,2)
    try
       data = [data,data_table{:,i}];
    catch
       data = [data, cellfun(@(x) str2double(x), data_table{:,i})];
    end
end

%% calcuate cent
roll_horizonR = zeros(size(data,1),1);
for i = horizon:size(data,1)
    roll_horizonR(i) =  exp(sum(data(i-horizon+1:i,1)))-1;
end
roll_horizonR(1:horizon-1)=[];
cevent = prctile(roll_horizonR,5);
...
srisk_not_floored = zeros(size(smpl,1),nf);
srisk = zeros(size(smpl,1),nf);

sriskv2_mean = zeros(size(smpl,1),1);
srisk_not_floored_mean = zeros(size(smpl,1),1);
srisk_mean = zeros(size(smpl,1),1);

for t = horizon + 1 :length(smpl)
%for t = 31:50
    ym = data(1:t,1); %market returns. Divide them by 100 if returns are introduced in percentage points 
    ...
    for i = 1:nf
        yi(:,i) = data(1:t,2+(i-1)*4); %return of firm i
    ...
    for i = 1:s1
        ...
        for k = 1:nf
            ...
            shortfall_srisk_i(i,k) = max(-kratio*dbt(end,k),kratio*dbt(end,k)-(1-kratio)*mvt(end,k)*(1+Ret_i_sim(i,k))); %Capital shortfall at the end of the simulated horizon under the SRISK definition
        end
        
    end
    
    for k = 1:nf
        if sum(Ret_m_sim<cevent) > 0
            sriskv2(t,k) = mean(shortfall_sriskv2_i(Ret_m_sim<cevent,k)); %SRISKv2 for firm i
            srisk_not_floored(t,k) = mean(shortfall_srisk_i(Ret_m_sim<cevent,k)); %SRISK for firm i before flooring at 0
            srisk(t,k) = max(0,srisk_not_floored(t,k)); %SRISK for firm i
        end
    end

    sriskv2_mean(t) = mean(sriskv2(t,:));
    srisk_not_floored_mean(t) = mean(srisk_not_floored(t,:));
    srisk_mean(t) = mean(srisk(t,:));

    fprintf([num2str(t),'/',num2str(length(smpl))]);
end
fprintf('Done!\n');

output_data = table(datestr(smpl),sriskv2,srisk,srisk_not_floored);
output_data_mean = table(datestr(smpl),sriskv2_mean,srisk_mean,srisk_not_floored_mean);

writetable(output_data,['srisk_data_detail',num2str(horizon),'.csv']);
writetable(output_data_mean,['srisk_data_mean',num2str(horizon),'.csv']);
