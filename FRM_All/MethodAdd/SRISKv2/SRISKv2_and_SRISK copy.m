...
for s=1:length(smpl)
    ...
    for k = 1:nf
        ...
        srisk_not_floored(s,k) = mean(shortfall_srisk_i(Ret_m_sim<cevent,k)); %SRISK for firm i before flooring at 0
        srisk(s,k) = max(0,srisk_not_floored(s,k)); %SRISK for firm i
    end
