...
# plot_labels_m = c(20201101, 20201115, 20201201, 20201215)
# plot_labels_y = c(20150601, 20160601, 20170601, 20180601, 20190601, 20200601)
...
# png(paste0(output_path, "/macro.png"), width = 900, height = 600, bg = "transparent")
# 
# macro$Date = as.Date(macro$Date)
# datebreaks <- seq(macro$Date[1], macro$Date[length(macro$Date)],
#                   by = "1 month")
# 
# print(ggplot()+geom_line(data = macro,aes(x = Date,y = Spread_InduBond_1T3Y,colour = "Spread_InduBond_1T3Y"),size=1)+
#         geom_line(data = macro,aes(x = Date,y = Spread_InduBond_3T5Y,colour = "Spread_InduBond_3T5Y"),size=1)+
#         geom_line(data = macro,aes(x = Date,y = Spread_InduBond_5T10Y,colour = "Spread_InduBond_5T10Y"),size=1)+
#         geom_line(data = macro,aes(x = Date,y = Spread_InduBond_Over10Y,colour = "Spread_InduBond_Over10Y"),size=1)+
#         xlab("Date")+ylab("spread")+ guides(fill=FALSE)+
...
#         ))
# 
# print(ggplot()+geom_line(data = macro,aes(x = Date,y = FXI.US.EQUITY,colour = "FXI.US.EQUITY"),size=1)+
#         geom_line(data = macro,aes(x = Date,y = VXFXI.INDEX,colour = "VXFXI.INDEX"),size=1)+
...
output_path = '/Users/wkhaerdle/Documents/Project/FRM-master_Modify_WRT/FRM_Quantlet/FRM_All/Output 20210727/Output/Asia'
address_Empty = '/Users/wkhaerdle/Documents/Project/FRM-master_Modify_WRT/FRM_Quantlet/FRM_All/Output 20210727/Output/Asia/Empty/Adj_Matrices_ShapleyCom/Lambda_DynamicFactor'
...
for (iFac in c(1:length(Macro_Total))){
  ...
  Shapley_Com_3 = abs(weight_3*(Shapley_3_plus-Shapley_3))
...
datebreaks <- seq(Shapley$Date[1], Shapley$Date[length(Shapley$Date)],
                  by = "1 month")

png(paste0(output_path, "/Shapley.png"), width = 900, height = 600, bg = "transparent")

print(ggplot()+geom_line(data = Shapley,aes(x = Date,y = FXI.US.EQUITY),colour = "red",size=1)+
        geom_line(data = Shapley,aes(x = Date,y = VXFXI.INDEX),colour = "blue",size=1)+
        geom_line(data = Shapley,aes(x = Date,y = CN2YR),colour = "black",size=1)+
        geom_line(data = Shapley,aes(x = Date,y = CN210SLOPE),colour = '#14B294',size=1)+
...
png(paste0(output_path, "/Shapley_Mean.png"), width = 900, height = 600, bg = "transparent")
print(ggplot(Shapley_Mean,aes(x=Macro,y=Shapley_Mean))+geom_bar(stat="identity",fill="steelblue")+
        theme(axis.text.x = element_text(angle = 30, hjust = 1), 
              panel.grid.major =element_blank(), 
              panel.grid.minor = element_blank(),
              panel.background = element_rect(fill = "transparent",colour = NA),
...
png(paste0(output_path, "/Shapley_Dynamic_Mean.png"), width = 900, height = 600, bg = "transparent")
print(ggplot()+geom_line(data = Shapley_R_Mean,aes(x = Date,y = Shapley_Mean),colour = "red",size=1)+
        xlab("Date")+ylab("Shapley Values")+
        scale_x_date(breaks = datebreaks) +
        theme(axis.text.x = element_text(angle = 30, hjust = 1), 
              panel.grid.major =element_blank(), 
...
print(ggplot()+geom_line(data = ShapleyRolling,aes(x = Date,y = FXI.US.EQUITY),colour = "red",size=1)+
        geom_line(data = ShapleyRolling,aes(x = Date,y = VXFXI.INDEX),colour = "blue",size=1)+
        geom_line(data = ShapleyRolling,aes(x = Date,y = CN2YR),colour = "black",size=1)+
...
for (t in N0_fixed:N1_fixed) { 
  ...
  data_fixed = cbind(stock_return[(t - s + 1):t, biggest_index_fixed],
               macro_return_add)
  # data_fixed = data_fixed[, colSums(data_fixed != 0) > 0]
...
# fun_loc = function(x){
#   loc = which(Total_Cor$id == x)
#   name = Total_Cor$name[loc]
#   sector = Total_Cor$sector[loc]
...
dir_T = "/Users/wkhaerdle/Documents/Project/FRM-master_Modify_WRT/FRM_Quantlet/FRM_All/Output 20210727/Output/Asia/CN2YR/Adj_Matrices_ShapleyCom/Comb_3_plus/1/"
...
# largest FRM on 20200203
# RiskCenter = data.frame(date = FRM_individ_fixed[,1], stock = rep(0, length(FRM_individ_fixed[,1])))
# for (day in N0_fixed_net:N1_fixed_net){
#   Judge = FRM_individ_fixed[day-N0_fixed_net+1,-1]
#   RiskCenter$stock[day - N0_fixed_net + 1] = colnames(FRM_individ_fixed)[which(Judge == max(Judge))+1]
# }
...
FRM_index1= read.csv2("/Users/wkhaerdle/Documents/Project/FRM-master_Modify_WRT/FRM_Quantlet/FRM_All/Input/Asia/20190102-20210210/FRM Asia time series/FRM_CHHKTW_Time_Series_FRM_2006-20200325.csv", header=F, encoding = "uft-8",sep = " ")
colnames(FRM_index1) = c("Date", "FRM")
FRM_index1$Date = gsub('[-]', '', FRM_index1$Date)
FRM_index1$Date = as.numeric(FRM_index1$Date)

FRM_index2 = read.csv("/Users/wkhaerdle/Documents/Project/FRM-master_Modify_WRT/FRM_Quantlet/FRM_All/Input/Asia/20190102-20210210/FRM Asia time series/FRM_CHHKTW_Time_Series_FRM_20201030_20200201.csv", header=F, encoding = "uft-8",sep = " ")
...
VIX = read_excel("/Users/wkhaerdle/Documents/Project/FRM-master_Modify_WRT/FRM_Quantlet/FRM_All/Input/Asia/20190102-20210210/VIX VXFXI.xlsx")
...
# # biggest top 3 sector
# ## 7. Macro influence
# dir.create(paste0(output_path, "/Macro"))
# M_Macro_Use = length(which(colnames(macro_return) %in%
#                              Macro_Total))
# # macro_inf = matrix(0, N_h, M_macro+1)
# macro_inf = matrix(0, N_h, M_Macro_Use+1)
# macro_inf[, 1] = names(FRM_history)
# colnames(macro_inf) = c("date", Macro_Total)
# macro_inf_MeanV = macro_inf
#   
# for (t in 1:N_h) {
...
#   k1 = ncol(adj0) - M_Macro_Use
#   # for (k in 1:M_macro) macro_inf[t, k+1] = sum(adj0[1:k1, k1+k]!=0)/k1
#   for (k in 1:M_Macro_Use) {
#     macro_inf[t, k+1] = sum(adj0[1:k1, k1+k]!=0)/k1
#     macro_inf_MeanV[t, k+1] = mean(abs(adj0[1:k1, k1+k]))
#   }
# }
# 
# write.csv(macro_inf, paste0(output_path, "/Macro/macro_influence.csv"))
# write.csv(macro_inf_MeanV, paste0(output_path, "/Macro/macro_influence_meanValue.csv"))
# 
# 
...
# 
# div = floor(nrow(FRM_index) / 4)
# plot_labels_y = c(FRM_index$date[div], 
#                   FRM_index$date[div*2], 
#                   FRM_index$date[div*3])
# 
...
#   scale_x_discrete(breaks = plot_labels_y, expand = c(0, 0)) +
#   scale_y_continuous(limits = c(0, as.numeric(max(macro_inf_long$inf_idx)))) +
...
# 
# # mean value
# macro_inf_long_value = gather(as.data.frame(macro_inf_MeanV), macro, inf_idx, -date, 
#                         convert = TRUE, factor_key = TRUE)
# 
# png(paste0(output_path, "/Macro/macro_inf_MeanV.png"), 
#     width = 900, height = 600, bg = "transparent")
# 
# time_div = macro_inf_MeanV[, 'date']
# 
# div = floor(nrow(macro_inf_MeanV) / 4)
# plot_labels_y = c(time_div[div], 
#                   time_div[div*2], 
#                   time_div[div*3])
# 
# ggplot(macro_inf_long_value, aes(date, as.numeric(inf_idx), group=macro)) +
#   geom_line(aes(color = macro), size = 1) + ylab("normalised # of non-zero betas") +
#   scale_x_discrete(breaks = plot_labels_y, expand = c(0, 0)) +
#   
#   theme(panel.background = element_rect(fill = "transparent", colour = NA),
...
# top 3 index
# 
# result = data.frame(macro_inf)
# result[, c(2:ncol(result))] = as.numeric(unlist(result[,c(2:ncol(result))]))
# result$date = as.numeric(result$date)
...
# }
# 
# png(paste0(output_path, "/Macro/Top3_macro_inf.png"), width = 900, height = 600, bg = "transparent")
# 
...
#         xlab("date")+ylab("normalised # of non-zero betas")+
#         scale_x_date(breaks = datebreaks) +
#         theme(axis.text.x = element_text(angle = 30, hjust = 1), 
#               panel.grid.major =element_blank(), 
#               panel.grid.minor = element_blank(),
...
# dev.off()
# 
# write.xlsx(Top3_Result, paste0(output_path, "/Macro/Top3_macro_inf.xlsx"), sheetName="Sheet1",
#            col.names=TRUE, row.names=TRUE, append=FALSE, showNA=TRUE)
...
#   geom_line(aes(color = macro), size = 1) + ylab("normalised # of non-zero betas") +
#   scale_x_discrete(breaks = plot_labels_y, expand = c(0, 0)) +
#   scale_y_continuous(limits = c(0, as.numeric(max(macro_inf_long_smooth$inf_idx)))) +
#   theme(panel.background = element_rect(fill = "transparent", colour = NA),
#         axis.line = element_line(colour = "black"),
#         axis.title.x = element_text(size=14),
#         axis.title.y = element_text(size=14),
...
# different tau
dir.create(paste0(output_path, "/Adj_Matrices_DynamicFactor/Tau"))
Tau_date = 20200120
# dynamic macro factors
macto_dir = "/Users/wkhaerdle/Documents/Project/FRM-master_Modify_WRT/FRM_Quantlet/FRM_All/Output/Asia/Adj_Matrices_test/quantile"
SelectFactor = read_excel(paste0(macto_dir, "/","MarcoScores_Quantile_IMP__Top3_20210512.xlsx")) # MarcoScores_Quantile_IMP__Top3_20210512 MarcoScores_Quantile_shapley__Top3_20210512
SelectFactor = SelectFactor[, -1]
SelectFactor$ticker = gsub('[-]', '', SelectFactor$Date)
SelectFactor$ticker = as.numeric(SelectFactor$ticker)
TimeRange =  as.numeric(gsub('[-]', '', mktcap$ticker))

for (tau in c(0.05, 0.1, 0.25, 0.5)) {
  timestart<-Sys.time()
  t = which(ticker==Tau_date)
  
  Ticker = TimeRange[t - 1]
  nloc = which(SelectFactor$ticker == Ticker)
  MacroName = c(SelectFactor$Top1_name[nloc], SelectFactor$Top2_name[nloc], SelectFactor$Top3_name[nloc])
  ...
  colnames(adj_matix) = colnames(data)
  rownames(adj_matix) = colnames(data)
  write.csv(adj_matix, paste0(output_path, "/Adj_Matrices_DynamicFactor/Tau/adj_matix_Base_", tau,"_",
                              Tau_date, ".csv"), quote = FALSE)
  # WRT change
  timeend<-Sys.time()
  runningtime<-timeend-timestart
  print(paste0(timeend, " runing ", runningtime," finish tao ", tau))
}
