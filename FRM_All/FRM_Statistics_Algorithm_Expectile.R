...
qrL1 <- function(Xd, a, max.steps, eps1 = 10^(-10), eps2 = 10^(-6), trace = T) {
  y = as.matrix(Xd[, 1])
  x = Xd[, 2 : ncol(Xd)]
  ### y in Real set 
  ### a_th quantile
...
###this defines the check function for expectile
pf <- function(beta0, y0, tau) {
  tmp <- y0 - beta0
  sum(tau*(tmp[tmp>0]^2)) +sum((1-tau)*(tmp[tmp<=0]^2))  
}
